...
func OIDCCallback(c *context.Context) {
	...
	if sessionState == nil || sessionState.(string) != state {
		c.RenderWithErr("Invalid state parameter", http.StatusBadRequest, tmplUserAuthLogin, nil)
		return
	}

	loginSourceID := c.Session.Get("oidc_login_source_id")
	if loginSourceID == nil {
		c.RenderWithErr("No login source in session", http.StatusBadRequest, tmplUserAuthLogin, nil)
		return
	}
	...
	code := c.Query("code")
	if code == "" {
		c.RenderWithErr("No authorization code received", http.StatusUnprocessableEntity, tmplUserAuthLogin, nil)
		return
	}
	...
	if err != nil {
		...
		c.RenderWithErr("Failed to authenticate with OIDC provider", http.StatusUnprocessableEntity, tmplUserAuthLogin, nil)
		return
	}
	...
	if user == nil {
		...
		if !oidcConfig.AutoRegister {
			c.RenderWithErr("User not found and auto-registration is disabled", http.StatusOK, tmplUserAuthLogin, nil)
			return
		}
